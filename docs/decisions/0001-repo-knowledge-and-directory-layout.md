...
- 换机器 pull 仓库即得全部项目知识。
- 目录布局难逆转，故留此 ADR。
- **连带待办**（✅ 2026-07-10 完成）：`v3-build/` 脚本原硬编码旧 `Downloads\` 路径已改为仓库相对（蓝图内无硬编码路径）。
- **另需补**（✅ 2026-07-10 完成）：已补 `AGENTS.md`（共享段与 `CLAUDE.md` 逐字节一致，`tools/check-discipline-drift.py` 校验）。
